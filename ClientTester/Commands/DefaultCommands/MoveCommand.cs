    ...
    public class MoveCommand : NitroxCommand
    {
        ...
        private void mouseTimerTick(MultiplayerClient client)
        {
            ...
            if (lastX != -1)
            {
                ...
                client.PacketSender.UpdatePlayerLocation(client.clientPos, velocity, Quaternion.identity, Quaternion.identity, Optional<VehicleModel>.Empty(), Optional<Guid>.Empty());
            }
            lastX = curX;
            lastY = curY;
        }
