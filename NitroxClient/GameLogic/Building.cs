...
namespace NitroxClient.GameLogic
{
    public class Building
    {
        ...
        public void PlaceBasePiece(ConstructableBase constructableBase, Base targetBase, TechType techType, Quaternion quaternion)
        {
            ...
            Guid guid = GuidHelper.GetGuid(constructableBase.gameObject);
            Guid parentBaseGuid = (targetBase == null) ? Guid.Empty : GuidHelper.GetGuid(targetBase.gameObject);
            Vector3 itemPosition = constructableBase.gameObject.transform.position;
            Transform camera = Camera.main.transform;

            PlaceBasePiece placedBasePiece = new PlaceBasePiece(packetSender.PlayerId, guid, itemPosition, quaternion, camera.position, camera.rotation, techType, Optional<Guid>.OfNullable(parentBaseGuid));
        ...
        public void PlaceFurniture(GameObject gameObject, TechType techType, Vector3 itemPosition, Quaternion quaternion)
        {
            ...
            Optional<Guid> subGuid = Optional<Guid>.Empty();
            var sub = Player.main.currentSub;
            if (sub != null)
            {
                subGuid = Optional<Guid>.Of(GuidHelper.GetGuid(sub.gameObject));
            }
        ...
        {
            Optional<Guid> newlyConstructedBaseGuid = Optional<Guid>.Empty();
            ...
            if (opConstructedBase.IsPresent())
            {
                ...
                newlyConstructedBaseGuid = Optional<Guid>.Of(GuidHelper.GetGuid(constructedBase));
            }
